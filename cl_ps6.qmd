...
filter_for_type = merged_df[(merged_df["type"] == ) & (merged_df["subtype"] == )]

filtered_for_type["binned_lat"] = np.floor(filtered_for_type["latitude"]/.01) * .01
filtered_for_type["binned_long"] = np.floor(filtered_for_type["longitude"]/.01) * .01
bin_count = (merged_df.groupby(["binned_lat", "binned_long"]).size().reset_index(name = "type_sub_count"))
top_10 = bin_count.nlargest(10, "type_sub_count")
```
